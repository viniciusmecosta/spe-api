...
class AsyncTimeRecordRepository(AsyncBaseRepository[TimeRecord, TimeRecordCreate, TimeRecordUpdate]):
    def __init__(self):
        super().__init__(TimeRecord)
    ...
    async def get(self, db: AsyncSession, record_id: int) -> TimeRecord | None:
        stmt = (
            select(TimeRecord)
            .options(
                selectinload(TimeRecord.user),
    ...
    async def get_last_by_user(self, db: AsyncSession, user_id: int) -> TimeRecord | None:
        stmt = (
            select(TimeRecord)
            .options(
                selectinload(TimeRecord.user),
    ...
    async def get_by_users_and_range(
            self, db: AsyncSession, user_ids: list[int], start_date: datetime, end_date: datetime
    ) -> list[TimeRecord]:
        stmt = (
            select(TimeRecord)
            .options(
    ...
    async def count_unique_users_in_range(
    ...
    async def update(
            self, db: AsyncSession, *, db_obj: TimeRecord, obj_in: TimeRecordUpdate | dict[str, Any]
    ) -> TimeRecord:
        return await super().update(db, db_obj=db_obj, obj_in=obj_in)

    async def delete(self, db: AsyncSession, record_id: int, manager_id: int):
        stmt = select(TimeRecord).where(
            TimeRecord.id == record_id,
            TimeRecord.is_ignored == False,
        )
    ...
    async def get_timeline(self, db: AsyncSession, record_id: int) -> list[TimeRecord]:
        stmt = select(TimeRecord).where(TimeRecord.id == record_id)
        result = await db.scalars(stmt)
        record = result.first()
        if not record:
        ...
        res_timeline = await db.scalars(timeline_stmt)
        return list(res_timeline.all())


time_record_repository = TimeRecordRepository()
async_time_record_repository = AsyncTimeRecordRepository()
