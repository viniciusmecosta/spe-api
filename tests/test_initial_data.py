...
def test_init_db_creates_user(db_session):
    with patch("app.features.users.user_repository.user_repository.get_by_username", return_value=None):
...
def test_init_db_user_exists(db_session):
    fake_user = MagicMock()
    with patch("app.features.users.user_repository.user_repository.get_by_username", return_value=fake_user):
        init_db(db_session)
