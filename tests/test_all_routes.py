...
def generate_report():
    ...
    for path, methods_dict in paths.items():
        ...
        valid_path = path.replace("{id}", "1").replace("{user_id}", "1").replace("{sensor_index}", "1").replace(
            "{record_id}", "1").replace("{version}", "1.0.0")
        invalid_path = path.replace("{id}", "invalid_str").replace("{user_id}", "invalid_str").replace("{sensor_index}",
                                                                                                       "abc").replace(
            "{record_id}", "x").replace("{version}", "invalid")
        ...
        for method_str, operation in methods_dict.items():
            ...
            for auth_name, headers in auth_scenarios.items():
                for p_name, payload in payloads:
                    ...
                    for path_variant, path_desc in [(valid_path, "Valid Path Params"),
                                                    (invalid_path, "Invalid Path Params")]:
