...
def test_get_dashboard(client: TestClient, mocker: MagicMock) -> None:
    ...
    mocker.patch.object(DashboardService, "get_dashboard_metrics", return_value=expected)
...
def test_get_my_dashboard(client: TestClient, mocker: MagicMock) -> None:
    ...
    mocker.patch.object(DashboardService, "get_my_dashboard", return_value=expected)
...
def test_get_my_history(client: TestClient, mocker: MagicMock) -> None:
    ...
    mocker.patch.object(ReportService, "get_history_report", return_value=expected)
...
def test_get_team_hours(client: TestClient, mocker: MagicMock) -> None:
    ...
    mocker.patch.object(DashboardService, "get_team_worked_hours", return_value=expected)
...
def test_export_monthly_report_excel(client: TestClient, mocker: MagicMock) -> None:
    ...
    mocker.patch.object(ReportService, "validate_excel_export_permission")
    mocker.patch.object(ExcelService, "generate_excel_report", return_value=fake_stream)
...
def test_get_user_detailed_report(client: TestClient, mocker: MagicMock) -> None:
    ...
    mocker.patch.object(ReportService, "get_advanced_user_report_or_404", return_value=expected)
