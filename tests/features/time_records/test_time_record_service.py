...
@pytest.mark.asyncio
async def test_get_my_records_and_list_records_for_admin(db_session_mock, mock_time_record_repo):
    mock_time_record_repo.get_all_by_user.return_value = ["rec1"]
    mock_time_record_repo.get_by_range.return_value = ["rec2"]

    assert await time_record_service.get_my_records(db_session_mock, 1) == ["rec1"]
    assert await time_record_service.list_records_for_admin(db_session_mock, 1, datetime(2026, 1, 1),
                                                      datetime(2026, 1, 31)) == ["rec2"]


@pytest.mark.asyncio
async def test_trigger_auto_print_enabled(db_session_mock, mocker):
...
@pytest.mark.asyncio
async def test_get_receipt_data_with_timeline(db_session_mock, mocker):
    ...
    mocker.patch.object(time_record_service, "get_record_timeline", new_callable=AsyncMock,
                        return_value=[timeline_mock])

    result = await time_record_service.get_receipt_data(db_session_mock, "valid", user)
    assert len(result.timeline) == 1
    assert result.timeline[0].action == "EDIT"


@pytest.mark.asyncio
async def test_get_receipt_pdf_errors(db_session_mock, mocker):
    ...
    with pytest.raises(InvalidReceiptIdError) as exc1:
        await time_record_service.get_receipt_pdf(db_session_mock, "invalid", user)
    ...
    with pytest.raises(TimeRecordNotFoundError) as exc2:
        await time_record_service.get_receipt_pdf(db_session_mock, "valid", user)
...
@pytest.mark.asyncio
async def test_validate_manual_punch_permission_user_not_found(db_session_mock, mock_user_repo):
    mock_user_repo.get.return_value = None
    request = MagicMock(spec=Request)
    with pytest.raises(TimeRecordUserNotFoundError) as exc_info:
        await time_record_service._validate_manual_punch_permission(db_session_mock, 1, request)
    assert exc_info.value.status_code == 404
    assert exc_info.value.detail == "Usuário não encontrado."


@pytest.mark.asyncio
async def test_validate_manual_punch_permission_manager(db_session_mock, mock_user_repo):
...
@pytest.mark.asyncio
async def test_validate_manual_punch_permission_mobile_allowed(db_session_mock, mock_user_repo):
...
@pytest.mark.asyncio
async def test_validate_manual_punch_permission_mobile_forbidden(db_session_mock, mock_user_repo):
...
@pytest.mark.asyncio
async def test_validate_manual_punch_permission_desktop_allowed(db_session_mock, mock_user_repo):
...
@pytest.mark.asyncio
async def test_validate_manual_punch_permission_desktop_forbidden(db_session_mock, mock_user_repo):
...
@pytest.mark.asyncio
@patch.object(time_record_service, "_validate_manual_punch_permission", new_callable=AsyncMock)
@patch("app.features.time_records.time_record_service.trusted_time_service.get_trusted_time")
async def test_register_entry_success(mock_get_trusted_time, mock_validate, db_session_mock, mock_payroll_service,
    ...
    result = await time_record_service.register_entry(db_session_mock, 1, request)
...
@patch.object(time_record_service, "_validate_manual_punch_permission", new_callable=AsyncMock)
@patch("app.features.time_records.time_record_service.trusted_time_service.get_trusted_time")
async def test_register_entry_ntp_fallback(mock_get_trusted_time, mock_validate, db_session_mock, mock_payroll_service,
                                     mock_get_client_ip, mock_get_client_device_name, mock_time_record_repo,
                                     mock_audit_service):
    current_time = datetime.now(ZoneInfo(settings.TIMEZONE))
...
@patch.object(time_record_service, "_validate_manual_punch_permission", new_callable=AsyncMock)
@patch("app.features.time_records.time_record_service.trusted_time_service.get_trusted_time")
async def test_register_exit_success(mock_get_trusted_time, mock_validate, db_session_mock, mock_payroll_service,
                               mock_get_client_ip, mock_get_client_device_name, mock_time_record_repo):
    current_time = datetime.now(ZoneInfo(settings.TIMEZONE))
    mock_get_trusted_time.return_value = (current_time, True)
    request = MagicMock(spec=Request)
    ...
    mock_time_record_repo.create.return_value = record
    result = await time_record_service.register_exit(db_session_mock, 1, request)
    assert result == record
    mock_validate.assert_called_once_with(db_session_mock, 1, request)
    mock_payroll_service.validate_period_open.assert_called_once_with(db_session_mock, current_time.date())
    mock_time_record_repo.create.assert_called_once_with(
...
@patch.object(time_record_service, "_validate_manual_punch_permission", new_callable=AsyncMock)
@patch("app.features.time_records.time_record_service.trusted_time_service.get_trusted_time")
async def test_register_exit_ntp_fallback(mock_get_trusted_time, mock_validate, db_session_mock, mock_payroll_service,
                                    mock_get_client_ip, mock_get_client_device_name, mock_time_record_repo,
                                    mock_audit_service):
    current_time = datetime.now(ZoneInfo(settings.TIMEZONE))
    mock_get_trusted_time.return_value = (current_time, False)
...
@pytest.mark.asyncio
async def test_toggle_record_type_not_found(db_session_mock, mock_time_record_repo):
    mock_time_record_repo.get.return_value = None
    user = User(id=1, role=UserRole.EMPLOYEE)
    with pytest.raises(TimeRecordNotFoundError) as exc_info:
        await time_record_service.toggle_record_type(db_session_mock, 1, user)
    assert exc_info.value.status_code == status.HTTP_404_NOT_FOUND
    assert "Registro de ponto (ID 1) não encontrado." in exc_info.value.detail


@pytest.mark.asyncio
async def test_toggle_record_type_forbidden(db_session_mock, mock_time_record_repo):
...
@pytest.mark.asyncio
async def test_toggle_record_type_success(db_session_mock, mock_time_record_repo, mock_payroll_service,
                                          mock_audit_service):
    ...
    mock_time_record_repo.get.return_value = record
    user = User(id=1, role=UserRole.MANAGER)
    result = await time_record_service.toggle_record_type(db_session_mock, 1, user)
...
@pytest.mark.asyncio
async def test_toggle_record_type_success_employee(db_session_mock, mock_time_record_repo, mock_payroll_service,
                                             mock_audit_service):
    dt = datetime.now(ZoneInfo(settings.TIMEZONE))
...
@pytest.mark.asyncio
async def test_create_admin_record(db_session_mock, mock_time_record_repo, mock_payroll_service, mock_audit_service):
    dt = datetime.now(ZoneInfo(settings.TIMEZONE))
    ...
    mock_time_record_repo.create.return_value = record
    result = await time_record_service.create_admin_record(db_session_mock, obj_in, manager_id=2,
                                                           ip_address="127.0.0.1",
                                                     device_name="Dev")
    assert result == record
...
@pytest.mark.asyncio
async def test_create_admin_record_no_device(db_session_mock, mock_time_record_repo, mock_payroll_service,
                                       mock_audit_service):
    dt = datetime.now(ZoneInfo(settings.TIMEZONE))
    obj_in = TimeRecordCreateAdmin(user_id=1, record_type=RecordType.ENTRY, record_datetime=dt, edit_justification="")
    record = TimeRecord(id=1, user_id=1)
    mock_time_record_repo.create.return_value = record
    await time_record_service.create_admin_record(db_session_mock, obj_in, manager_id=2, ip_address="127.0.0.1",
                                            device_name=None)
    mock_time_record_repo.create.assert_called_once_with(db_session_mock, user_id=1, record_type=RecordType.ENTRY,
...
@pytest.mark.asyncio
async def test_update_admin_record_not_found(db_session_mock, mock_time_record_repo):
    mock_time_record_repo.get.return_value = None
    obj_in = TimeRecordUpdate(edit_justification="missing")
    with pytest.raises(TimeRecordNotFoundError) as exc_info:
        await time_record_service.update_admin_record(db_session_mock, 1, obj_in, 2)
    assert exc_info.value.status_code == 404
    assert "Registro de ponto (ID 1) não encontrado." in exc_info.value.detail


@pytest.mark.asyncio
async def test_update_admin_record_no_changes(db_session_mock, mock_time_record_repo, mock_payroll_service):
    dt = datetime.now(ZoneInfo(settings.TIMEZONE))
    record = TimeRecord(id=1, record_type=RecordType.ENTRY, record_datetime=dt)
    mock_time_record_repo.get.return_value = record
    obj_in = TimeRecordUpdate(edit_justification="")
    result = await time_record_service.update_admin_record(db_session_mock, 1, obj_in, 2)
    assert result == record


@pytest.mark.asyncio
async def test_update_admin_record_with_changes(db_session_mock, mock_time_record_repo, mock_payroll_service,
                                          mock_audit_service):
    dt = datetime.now(ZoneInfo(settings.TIMEZONE))
    ...
    result = await time_record_service.update_admin_record(db_session_mock, 1, obj_in, 2, "1.1.1.1", "Dev", "Plat")
    assert result.record_type == RecordType.EXIT
...
@pytest.mark.asyncio
async def test_update_admin_record_no_device(db_session_mock, mock_time_record_repo, mock_payroll_service,
                                       mock_audit_service):
    dt = datetime.now(ZoneInfo(settings.TIMEZONE))
    record = TimeRecord(id=1, user_id=1, record_type=RecordType.ENTRY, record_datetime=dt)
    mock_time_record_repo.get.return_value = record
...
@pytest.mark.asyncio
async def test_delete_admin_record_not_found(db_session_mock, mock_time_record_repo):
    mock_time_record_repo.get.return_value = None
    obj_in = TimeRecordDeleteAdmin(edit_justification="Err")
    with pytest.raises(TimeRecordNotFoundError) as exc_info:
        await time_record_service.delete_admin_record(db_session_mock, 1, obj_in, 2)
    assert exc_info.value.status_code == 404
    assert "Registro de ponto (ID 1) não encontrado." in exc_info.value.detail


@pytest.mark.asyncio
async def test_delete_admin_record_success(db_session_mock, mock_time_record_repo, mock_payroll_service,
                                           mock_audit_service):
    dt = datetime.now(ZoneInfo(settings.TIMEZONE))
    record = TimeRecord(id=1, record_type=RecordType.ENTRY, record_datetime=dt)
    mock_time_record_repo.get.return_value = record
...
@pytest.mark.asyncio
async def test_delete_admin_record_no_justification(db_session_mock, mock_time_record_repo, mock_payroll_service,
                                              mock_audit_service):
    dt = datetime.now(ZoneInfo(settings.TIMEZONE))
    record = TimeRecord(id=1, record_type=RecordType.ENTRY, record_datetime=dt)
    mock_time_record_repo.get.return_value = record
...
async def test_create_punch_first_record(mock_get_device, db_session_mock, mock_time_record_repo):
    mock_time_record_repo.get_last_by_user.return_value = None
    mock_get_device.return_value = "Dev"
    dt = datetime.now(ZoneInfo(settings.TIMEZONE))
    record = TimeRecord(id=1)
    mock_time_record_repo.create.return_value = record
    result = await time_record_service.create_punch(db_session_mock, 1, dt, "1.1.1.1")
    assert result == record
    mock_time_record_repo.create.assert_called_once_with(db_session_mock, user_id=1, record_type=RecordType.ENTRY,
...
@pytest.mark.asyncio
@patch("app.features.time_records.time_record_service.get_client_device_name")
async def test_create_punch_same_day_exit(mock_get_device, db_session_mock, mock_time_record_repo):
    ...
    await time_record_service.create_punch(db_session_mock, 1, dt_curr, "1.1.1.1", biometric_id=123)
...
@pytest.mark.asyncio
@patch("app.features.time_records.time_record_service.get_client_device_name")
async def test_create_punch_different_day(mock_get_device, db_session_mock, mock_time_record_repo):
    dt_last = datetime(2023, 1, 1, tzinfo=ZoneInfo("UTC"))
    last_record = TimeRecord(id=1, record_type=RecordType.ENTRY, record_datetime=dt_last)
    mock_time_record_repo.get_last_by_user.return_value = last_record
    dt_curr = datetime(2023, 1, 2, tzinfo=ZoneInfo("UTC"))
    mock_time_record_repo.create.return_value = TimeRecord(id=2)
    await time_record_service.create_punch(db_session_mock, 1, dt_curr, "1.1.1.1")
    mock_time_record_repo.create.assert_called_once_with(db_session_mock, user_id=1, record_type=RecordType.ENTRY,
                                                         record_datetime=dt_curr, ip_address="1.1.1.1",
...
@pytest.mark.asyncio
@patch("app.features.time_records.time_record_service.get_client_device_name")
async def test_create_punch_naive_datetimes(mock_get_device, db_session_mock, mock_time_record_repo):
...
async def test_get_record_timeline(db_session_mock, mock_time_record_repo):
    ...
    result = await time_record_service.get_record_timeline(db_session_mock, 1)
    assert result == records
    mock_time_record_repo.get_timeline.assert_called_once_with(db_session_mock, 1)


@pytest.mark.asyncio
async def test_invalidate_extra_time_requests(db_session_mock):
...
@pytest.mark.asyncio
async def test_is_first_entry_affected_no_entries(db_session_mock):
    mock_query = MagicMock()
    mock_filter = MagicMock()
...
@pytest.mark.asyncio
async def test_is_first_entry_affected_matches_record_id(db_session_mock):
...
@pytest.mark.asyncio
async def test_is_first_entry_affected_new_datetime_earlier(db_session_mock):
    first_entry = TimeRecord(id=10, record_datetime=datetime(2023, 1, 1, 8, 0, tzinfo=ZoneInfo("UTC")))
    mock_query = MagicMock()
    mock_filter = MagicMock()
    mock_order = MagicMock()
...
@pytest.mark.asyncio
async def test_is_first_entry_affected_new_datetime_later(db_session_mock):
    first_entry = TimeRecord(id=10, record_datetime=datetime(2023, 1, 1, 8, 0, tzinfo=ZoneInfo("UTC")))
    mock_query = MagicMock()
    mock_filter = MagicMock()
    mock_order = MagicMock()
...
@pytest.mark.asyncio
async def test_get_receipt_data_invalid_hashid(db_session_mock, mocker):
    mocker.patch("app.shared.hashid_service.hashid_service.decode", return_value=None)
    user = User(id=1, role=UserRole.EMPLOYEE)
    with pytest.raises(InvalidReceiptIdError) as exc_info:
        await time_record_service.get_receipt_data(db_session_mock, "invalid", user)
    assert exc_info.value.status_code == 404
    assert "invalid" in exc_info.value.detail


@pytest.mark.asyncio
async def test_get_receipt_data_record_not_found(db_session_mock, mocker):
    mocker.patch("app.shared.hashid_service.hashid_service.decode", return_value=123)
    mocker.patch("app.features.time_records.time_record_service.time_record_repository.get", return_value=None)
    user = User(id=1, role=UserRole.EMPLOYEE)
    with pytest.raises(TimeRecordNotFoundError) as exc_info:
        await time_record_service.get_receipt_data(db_session_mock, "valid", user)
    assert exc_info.value.status_code == 404
    assert "Registro de ponto (ID 123) não encontrado." in exc_info.value.detail


@pytest.mark.asyncio
async def test_get_receipt_data_forbidden_employee(db_session_mock, mocker):
    mocker.patch("app.shared.hashid_service.hashid_service.decode", return_value=123)
...
@pytest.mark.asyncio
async def test_get_receipt_data_success(db_session_mock, mocker):
    mocker.patch("app.shared.hashid_service.hashid_service.decode", return_value=123)
    user = User(id=1, name="John", cpf="12345678900", pis="12345", role=UserRole.EMPLOYEE)
    mock_record = TimeRecord(id=123, user_id=1, user=user, record_datetime=datetime.now(ZoneInfo(settings.TIMEZONE)),
                             record_type=RecordType.ENTRY, device_name="Device 1")
...
@pytest.mark.asyncio
async def test_get_receipt_pdf_success(db_session_mock, mocker):
    mocker.patch("app.shared.hashid_service.hashid_service.decode", return_value=123)
    ...
    pdf_bytes, filename = await time_record_service.get_receipt_pdf(db_session_mock, "valid", user)
...
@pytest.mark.asyncio
async def test_trigger_auto_print_disabled(db_session_mock, mocker):
    mock_company = MagicMock()
