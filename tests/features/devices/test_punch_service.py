...
@pytest.mark.asyncio
async def test_process_biometric_punch_propagates_device_name_from_state(async_db_mock, mocker):
    bio = UserBiometric(id=1, user=User(id=1, is_active=True))
    record = TimeRecord(id=1)
    ...
    req = MagicMock()
    req.state.device_name = "Catraca-Principal"
...
@pytest.mark.asyncio
async def test_process_biometric_punch_propagates_device_name_from_header_when_no_state(async_db_mock, mocker):
    bio = UserBiometric(id=1, user=User(id=1, is_active=True))
    record = TimeRecord(id=1)
    ...
    req = MagicMock()
    del req.state
    req.headers.get.side_effect = lambda k, d=None: "Header-Device" if k == "X-Device-Name" else d
