...
def test_check_unapproved_adjustments():
    ...
    adj1 = MockAdjustmentRequest(1, current_date, AdjustmentType.EXTRA_TIME, AdjustmentStatus.PENDING, 2.5,
    ...
    anomalies2 = anomaly_service._check_unapproved_adjustments(1, "Test", current_date, [adj2],
                                                               datetime(2023, 1, 1, 9, 0).time())
    assert len(anomalies2) == 1
    assert "0 minutos extras pendentes de aprovação" in anomalies2[0].description

    adj_pending_no_time = MockAdjustmentRequest(1, current_date, AdjustmentType.EXTRA_TIME, AdjustmentStatus.PENDING,
                                                1.0, None)
    anomalies_pending_no_time = anomaly_service._check_unapproved_adjustments(1, "Test", current_date,
                                                                              [adj_pending_no_time], None)
    ...
    adj4 = MockAdjustmentRequest(1, current_date, AdjustmentType.EXTRA_TIME, AdjustmentStatus.REJECTED, 1.5,
                                 datetime(2023, 1, 1, 19, 0).time())
...
async def test_get_anomalies_with_user(mock_tr_repo, mock_user_repo, db_session_mock):
    ...
    adj = MockAdjustmentRequest(1, date(2023, 1, 1), AdjustmentType.EXTRA_TIME, AdjustmentStatus.PENDING, 2.5,
                                datetime(2023, 1, 1, 18, 0).time())
