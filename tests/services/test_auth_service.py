...
def test_authenticate_user_not_found(db_session_mock: MagicMock, mocker: MagicMock) -> None:
    mocker.patch("app.services.auth_service.user_repository.get_by_username", return_value=None)
...
def test_authenticate_password_mismatch(db_session_mock: MagicMock, mocker: MagicMock) -> None:
    ...
    mocker.patch("app.services.auth_service.user_repository.get_by_username", return_value=user)
    mocker.patch("app.services.auth_service.settings.ENVIRONMENT", "prod")
    mocker.patch("app.services.auth_service.security.verify_password", return_value=False)
...
def test_authenticate_inactive_user(db_session_mock: MagicMock, mocker: MagicMock) -> None:
    ...
    mocker.patch("app.services.auth_service.user_repository.get_by_username", return_value=user)
    mocker.patch("app.services.auth_service.settings.ENVIRONMENT", "prod")
    mocker.patch("app.services.auth_service.security.verify_password", return_value=True)
...
def test_authenticate_dev_bypass_success(db_session_mock: MagicMock, mocker: MagicMock) -> None:
    ...
    user.is_active = True
    mocker.patch("app.services.auth_service.user_repository.get_by_username", return_value=user)
    mocker.patch("app.services.auth_service.settings.ENVIRONMENT", "dev")
    mocker.patch("app.services.auth_service.security.create_access_token", return_value="fake_token")
    mocker.patch("app.services.auth_service.audit_service.log")
...
def test_authenticate_success(db_session_mock: MagicMock, mocker: MagicMock) -> None:
    ...
    mocker.patch("app.services.auth_service.settings.ENVIRONMENT", "prod")
    mocker.patch("app.services.auth_service.security.verify_password", return_value=True)
    mocker.patch("app.services.auth_service.security.create_access_token", return_value="prod_token")
    mocker.patch("app.services.auth_service.audit_service.log")
