...
def test_get_trusted_time_success(mock_ntp_client):
    ...
    time, is_trusted = time_record_service._get_trusted_time()

    assert is_trusted is True
    assert isinstance(time, datetime)
    assert time.tzinfo == ZoneInfo(settings.TIMEZONE)
...
@patch.object(time_record_service, "_get_trusted_time")
def test_register_entry_success(
    mock_get_trusted_time, mock_validate, db_session_mock, mock_payroll_service,
    ...
    mock_time_record_repo.create.assert_called_once_with(
        db_session_mock, 1, RecordType.ENTRY, current_time, "127.0.0.1", "TestDevice", platform="desktop"
    )


@patch.object(time_record_service, "_validate_manual_punch_permission")
@patch.object(time_record_service, "_get_trusted_time")
def test_register_entry_ntp_fallback(
    mock_get_trusted_time, mock_validate, db_session_mock, mock_payroll_service,
    mock_get_client_ip, mock_get_client_device_name, mock_time_record_repo, mock_audit_service
):
    current_time = datetime.now(ZoneInfo(settings.TIMEZONE))
...
@patch.object(time_record_service, "_validate_manual_punch_permission")
@patch.object(time_record_service, "_get_trusted_time")
def test_register_exit_success(
    mock_get_trusted_time, mock_validate, db_session_mock, mock_payroll_service,
    mock_get_client_ip, mock_get_client_device_name, mock_time_record_repo
):
    current_time = datetime.now(ZoneInfo(settings.TIMEZONE))
    ...
    mock_payroll_service.validate_period_open.assert_called_once_with(db_session_mock, current_time.date())
    mock_time_record_repo.create.assert_called_once_with(
        db_session_mock, 1, RecordType.EXIT, current_time, "127.0.0.1", "TestDevice", platform="desktop"
    )


@patch.object(time_record_service, "_validate_manual_punch_permission")
@patch.object(time_record_service, "_get_trusted_time")
def test_register_exit_ntp_fallback(
    mock_get_trusted_time, mock_validate, db_session_mock, mock_payroll_service,
    mock_get_client_ip, mock_get_client_device_name, mock_time_record_repo, mock_audit_service
):
    current_time = datetime.now(ZoneInfo(settings.TIMEZONE))
...
def test_create_admin_record(db_session_mock, mock_time_record_repo, mock_payroll_service, mock_audit_service):
    dt = datetime.now(ZoneInfo(settings.TIMEZONE))
    obj_in = TimeRecordCreateAdmin(
        user_id=1, record_type=RecordType.ENTRY, record_datetime=dt, edit_justification="Forgot"
    )
    record = TimeRecord(id=1, user_id=1)
    mock_time_record_repo.create.return_value = record

    result = time_record_service.create_admin_record(
        db_session_mock, obj_in, manager_id=2, ip_address="127.0.0.1", device_name="Dev"
    )
    ...
    assert result.is_verified is True
    mock_time_record_repo.create.assert_called_once_with(
        db_session_mock, user_id=1, record_type=RecordType.ENTRY,
        record_datetime=dt, ip_address="127.0.0.1", device_name="Dev", platform="WEB_ADMIN"
    )
    db_session_mock.add.assert_called_once_with(record)
    db_session_mock.commit.assert_called_once()
    db_session_mock.refresh.assert_called_once_with(record)
...
def test_create_admin_record_no_device(db_session_mock, mock_time_record_repo, mock_payroll_service, mock_audit_service):
    dt = datetime.now(ZoneInfo(settings.TIMEZONE))
    obj_in = TimeRecordCreateAdmin(
        user_id=1, record_type=RecordType.ENTRY, record_datetime=dt, edit_justification=""
    )
    record = TimeRecord(id=1, user_id=1)
    mock_time_record_repo.create.return_value = record

    time_record_service.create_admin_record(
        db_session_mock, obj_in, manager_id=2, ip_address="127.0.0.1", device_name=None
    )
    mock_time_record_repo.create.assert_called_once_with(
        db_session_mock, user_id=1, record_type=RecordType.ENTRY,
        record_datetime=dt, ip_address="127.0.0.1", device_name="", platform="WEB_ADMIN"
    )
...
def test_update_admin_record_with_changes(db_session_mock, mock_time_record_repo, mock_payroll_service, mock_audit_service):
    ...
    record = TimeRecord(
        id=1, user_id=1, record_type=RecordType.ENTRY, record_datetime=dt,
        edit_justification=None, original_record_id=5
    )
    mock_time_record_repo.get.return_value = record
...
def test_create_punch_same_day_exit(mock_get_device, db_session_mock, mock_time_record_repo):
    ...
    mock_time_record_repo.create.assert_called_once_with(
        db_session_mock, user_id=1, record_type=RecordType.EXIT,
        record_datetime=dt_curr, ip_address="1.1.1.1", device_name="Dev",
        platform="desktop", biometric_id=123
    )
...
def test_create_punch_different_day(mock_get_device, db_session_mock, mock_time_record_repo):
    ...
    mock_time_record_repo.create.assert_called_once_with(
        db_session_mock, user_id=1, record_type=RecordType.ENTRY,
        record_datetime=dt_curr, ip_address="1.1.1.1", device_name=mock_get_device.return_value,
        platform="desktop", biometric_id=None
    )
