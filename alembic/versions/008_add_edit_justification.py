...
def upgrade() -> None:
    ...
    with op.batch_alter_table('time_records', schema=None) as batch_op:
        if 'edit_justification' not in columns:
            batch_op.add_column(sa.Column('edit_justification', sa.Enum('FORGOT_ENTRY', 'FORGOT_EXIT', 'SYSTEM_ERROR', 'INITIAL_INCLUSION', 'INITIAL_EDIT', 'REGISTRATION_MISTAKE', 'IRRELEVANT_RECORD', 'OTHER', name='editjustification'), nullable=True))
